...
impl ProtocolDeserializable<MetadataResponse> for Vec<u8> {
    fn into_protocol_type(self) -> ProtocolDeserializeResult<MetadataResponse> {
        let result = de_i32(self[0..4].to_vec()).and_then(|throttle_time_ms| {
            de_array(self[4..].to_vec(), deserialize_broker_metadata).map(
                |(brokers, remaining_bytes): (Vec<BrokerMetadata>, Vec<u8>)| {
        ...
        let result = result.and_then(|(throttle_time_ms, brokers, cluster_id, remaining_bytes)| {
            de_i32(remaining_bytes[0..4].to_vec()).map(|controller_id| {
                (
                    ...
                    cluster_id,
                    controller_id,
                    remaining_bytes[4..].to_vec(),
                )
            })
        });

        let result = result.and_then(
            |(throttle_time_ms, brokers, cluster_id, controller_id, remaining_bytes)| {
                de_array(remaining_bytes, deserialize_topic_metadata).map(
                    |(topic_metadata, remaining_bytes)| {
                        (
                            throttle_time_ms,
                            brokers,
                            cluster_id,
        ...
        result.map(
            |(
                ...
                if !remaining_bytes.is_empty() {
                    panic!(
                        "Deserialize of MetadataResponse did not cover remaining {} bytes",
                        remaining_bytes.len()
                    );
                }
                MetadataResponse {
...
fn deserialize_broker_metadata(
    bytes: Vec<u8>,
) -> ProtocolDeserializeResult<DynamicSize<BrokerMetadata>> {
    let result = de_i32(bytes[0..4].to_vec()).and_then(|node_id| {
    ...
    let result = result.and_then(|(node_id, host, remaining_bytes)| {
        de_i32(remaining_bytes[0..4].to_vec()).and_then(|port| {
            de_string(remaining_bytes[4..].to_vec())
                .map(|(rack, remaining_bytes)| (node_id, host, port, rack, remaining_bytes))
        })
    });
...
fn deserialize_topic_metadata(
    bytes: Vec<u8>,
) -> ProtocolDeserializeResult<DynamicSize<TopicMetadata>> {
    de_i16(bytes[0..2].to_vec()).and_then(|error_code| {
        de_string(bytes[2..].to_vec())
            ...
            .and_then(|(topic, is_internal, remaining_bytes)| {
                de_array(remaining_bytes, deserialize_partition_metadata).map(
                    |(partition_metadata, remaining_bytes)| {
                        (
                            TopicMetadata {
                                error_code,
                                topic,
...
fn deserialize_partition_metadata(
    bytes: Vec<u8>,
) -> ProtocolDeserializeResult<DynamicSize<PartitionMetadata>> {
    de_i16(bytes[0..2].to_vec()).and_then(|error_code| {
            ...
            de_i32(bytes[6..10].to_vec()).and_then(|leader| {
                ...
                .and_then(|(replicas, remaining_bytes)| {
                    ...
                    .and_then(|(isr, remaining_bytes)| {
                        de_array(remaining_bytes, |bytes| {
                            de_i32(bytes[0..4].to_vec())
                                .map(|offline_replicas| (offline_replicas, bytes[4..].to_vec()))
                        })
