    ...
    fn into_protocol_type(self) -> ProtocolDeserializeResult<MetadataResponse> {
        let result = de_i32(self[0..4].to_vec()).and_then(|throttle_time_ms| {
            de_array(self[4..].to_vec(), deserialize_broker_metadata)
                .map(|(brokers, remaining_bytes): (Vec<BrokerMetadata>, Vec<u8>)| (throttle_time_ms, brokers, remaining_bytes))
        });
        ...
        let result = result.and_then(|(throttle_time_ms, brokers, cluster_id, remaining_bytes)| {
            de_i32(remaining_bytes[0..4].to_vec())
                .map(|controller_id| (throttle_time_ms, brokers, cluster_id, controller_id, remaining_bytes[4..].to_vec()))
        });
...
fn deserialize_broker_metadata(bytes: Vec<u8>) -> ProtocolDeserializeResult<DynamicSize<BrokerMetadata>> {
    ...
    let result = result.and_then(|(node_id, host, remaining_bytes)| {
        de_i32(remaining_bytes[0..4].to_vec())
            .and_then(|port| de_string(remaining_bytes[4..].to_vec()).map(|(rack, remaining_bytes)| (node_id, host, port, rack, remaining_bytes)))
    });
    ...
    de_i16(bytes[0..2].to_vec()).and_then(|error_code| {
        de_i32(bytes[2..6].to_vec()).and_then(|partition| {
            de_i32(bytes[6..10].to_vec()).and_then(|leader| {
                de_array(bytes[10..].to_vec(), |bytes| de_i32(bytes[0..4].to_vec()).map(|replicas| (replicas, bytes[4..].to_vec()))).and_then(
                    |(replicas, remaining_bytes)| {
                        de_array(remaining_bytes, |bytes| de_i32(bytes[0..4].to_vec()).map(|isr| (isr, bytes[4..].to_vec()))).and_then(
                            |(isr, remaining_bytes)| {
