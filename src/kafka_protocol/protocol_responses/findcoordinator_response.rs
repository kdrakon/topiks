        ...
        de_i32(self[0..=3].to_vec()).and_then(|throttle_time_ms| {
            de_i16(self[4..=5].to_vec()).and_then(|error_code| {
                de_string(self[6..].to_vec()).and_then(|(error_message, bytes)| {
                    bytes.into_protocol_type().map(|coordinator: Coordinator| FindCoordinatorResponse {
                        throttle_time_ms,
                        error_code,
                        error_message,
                        coordinator,
        ...
        de_i32(self[0..=3].to_vec()).and_then(|node_id| {
            de_string(self[4..].to_vec()).and_then(|(host, bytes)| {
                host.ok_or(DeserializeError::of("Unexpected null host for Coordinator"))
                    .and_then(|host| de_i32(bytes[0..=3].to_vec()).map(|port| Coordinator { node_id, host, port }))
            })
        })
    }
}
