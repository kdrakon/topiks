...
fn show_dialog_message(screen: &mut impl Write, width: u16, message: &Option<DialogMessage>) {
    ...
    match message.as_ref() {
        ...
        Some(DialogMessage::Error(error)) => write!(
            screen,
            "{}{}",
            color::Bg(color::LightRed),
            pad_right(&error, width)
        )
        .unwrap(),
        Some(DialogMessage::Warn(warn)) => write!(
            screen,
            "{}{}",
            color::Bg(color::LightYellow),
            pad_right(&warn, width)
        )
        .unwrap(),
        Some(DialogMessage::Info(info)) => write!(
            screen,
            "{}{}",
            color::Bg(color::LightBlue),
            pad_right(&info, width)
        )
        .unwrap(),
    }
...
fn show_topic_partitions(
    ...
    if let Some((page_index, page)) = paged.page(partition_info_state.selected_index) {
        let indexed = page
            .iter()
            .zip(0..page.len())
            .collect::<Vec<(&&PartitionMetadata, usize)>>();
        let list_items = indexed
            .iter()
            .map(|&(partition_metadata, index)| {
                let consumer_offset = partition_info_state
                    .consumer_offsets
                    .get(&partition_metadata.partition)
                    .map(|p| p.offset)
                    .unwrap_or(-1);
                let partition_offset = partition_info_state
                    .partition_offsets
                    .get(&partition_metadata.partition)
                    .map(|p| p.offset)
                    .unwrap_or(-1);
...
fn show_topic_info(
    ...
    write!(
        ...
        pad_right(
            &format!(
                "Internal: {}",
                &(utils::bool_yes_no(topic_metadata.is_internal))
            ),
            width,
        )
    )
    ...
    write!(
        ...
        "{}{}{}",
        style::Bold,
        pad_right(&String::from("Configs:"), width),
        style::Reset
    )
    .unwrap();
    let longest_config_name_len = config_resource
        .config_entries
        .iter()
        .map(|config_entry| config_entry.config_name.len())
        .max()
        .unwrap_or(0) as u16;
    ...
    if let Some((page_index, page)) = paged.page(topic_info.selected_index) {
        let indexed = page
            .iter()
            .zip(0..page.len())
            .collect::<Vec<(&&ConfigEntry, usize)>>();
        let list_items = indexed
            .iter()
            .map(|&(config_entry, index)| {
                let item = Config {
                    name: pad_right(&config_entry.config_name, longest_config_name_len),
                    value: config_entry.config_value.clone(),
                };
                let item = if page_index == index {
                    Selected(Box::from(item))
                } else {
                    item
                };
                let item = if config_entry.config_source == ConfigSource::TopicConfig as i8 {
                    Override(Box::from(item))
                } else {
                    item
                };
                let item = if topic_info
                    .configs_marked_deleted
                    .contains(&config_entry.config_name)
                {
                    Deleted(Box::from(item))
                } else {
                    item
                };
                let item = if topic_info
                    .configs_marked_modified
                    .contains(&config_entry.config_name)
                {
                    Modified(Box::from(item))
                } else {
                    item
                };
                item
